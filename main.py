import requests, lxml.html as lh, numpy as np, pandas as pd, os, socket, sys
...
@app.route('/')
def home():
  file = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'data', 'worldometer_covid_dataset.csv')
...
def scrape(file, format='xlsx', debug=True):
    ...
    if format == 'xlsx' or format == 'xls':
        df = pd.read_excel(f'{file}.{format}')  if os.path.isfile(f'{file}.{format}') else None
    else:
        df = pd.read_csv(f'{file}.{format}') if os.path.isfile(f'{file}.{format}') else None
  ...
  try:
      while True:
          if socket.gethostbyname(socket.gethostname()) != '127.0.0.1':
              s = perf_counter()
              file = scrape(file, 'csv', False)
              e = perf_counter()
              t = e - s
              print(f'Time Taken -> {round(t, 2)} seconds')
              sleep(300)
          else:
              sleep(1)
              print('.'*i, end='\r')
